/// File Name: OverdrivePowerupControl.cs
/// File Author(s): Lincoln Schroeder
/// File Purpose: Every Overdrive Powerup uses this script for handling where to move
/// and what to do when the player picks it up.
/// 
/// Date Last Updated: November 26, 2019

using UnityEngine;

public class OverdrivePowerupControl : MonoBehaviour
{
    [Header("Movement Utilities")]
    [SerializeField]
    private float moveSpeed;
    [SerializeField]
    private GameObject overdrivePrefab;

    [Header("Sounds")]
    [SerializeField]
    private AudioClip overdriveActivated;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            GameObject.Instantiate(overdrivePrefab);
            persistentController.GetComponent<AudioSource>().PlayOneShot(overdriveActivated);
            Destroy(this.gameObject);
        }
    }
}
