...
public class PlayerControl : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!persistentController.IsPaused() && player != null)
        {
    ...
    /// <summary>
    /// Checks if the player has somehow travelled way outside of the play area.
    /// This only happens in custom stages where the player can leave the play zone.
    /// </summary>
    /// <returns>True if the player is outside of bounds, false otherwise.</returns>
    private bool CheckIfWayOffScreen()
    {
        if (player.transform.position.x < -50 || player.transform.position.x > 150
            || player.transform.position.y < -50 || player.transform.position.y > 150)
            return true;
        return false;
    }
}
